...
public class ChessMatch {
	...
	public boolean[][] possibleMoves(ChessPosition sourceChessPosition) {
		Position position = sourceChessPosition.toPosition();
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("There choses piece can't move to target position");
		}
	}
	...
	private Color opponent(Color color) {
		return (color == color.WHITE) ? Color.BLACK.BLACK : Color.WHITE;
	}
			...
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) {
						...
						Piece capPturePiece = makeMove(source, target);
						boolean testeCheck = testCheck(color);
						undoMove(source, target, capPturePiece);
						if (!testeCheck) {
							return false;
						}
					}
